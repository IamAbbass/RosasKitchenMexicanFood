                                        ...
                                        <div class="card radius-15">
                                            <div class="card-body text-center border radius-15" style="padding: 0; {{ $product->is_available == 0 ? "border-color: red !important":"" }}">
                                                ...
                                                <b>
                                                    ...
                                                    <p class="mb-0 hidden-view">____ / {{ $product->unit->name }}</p>
                                                </b>
                                                <input type="number" name="purchase[]" value="{{ $product->purchase }}" class="form-control mb-1 small_field hidden-print" title="Purchase">
                                                <input type="number" name="sale[]" value="{{ $product->sale }}" class="form-control small_field hidden-print" title="Sale">
                                                <select name="unit_id[]" class="form-control extra_small_field hidden-print" title="Unit">
                                                    @foreach($units as $unit)
                                                        <option {{ ($product->unit_id == $unit->id) ? 'selected' : '' }} value="{{$unit->id}}">{{ $unit->name }}</option>
                                                    @endforeach
                                                </select>
                                                <input type="number" name="discount[]" value="{{ $product->discount }}" class="form-control small_field hidden-print" title="Discount">
                                            </div>
                                        </div>
                                    </div>
                                    @endforeach
