        ...
        <div class="card-body">
            ...
            <div class="row">
                <div class="col-md-7 table-responsive">
                    ...
                    <table class="table table-striped table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    {{ $order_detail->product->name }},<br>{{ $order_detail->product->name_ru }}, {{ $order_detail->product->name_ur }}
                                    <hr style="margin: 0.2rem 0 !important">
                                    Profit: {{ ($order_detail->sale - $order_detail->purchase) * $order_detail->quantity }}
                            </td>
                                <td>
                                    <input type="number" name="purchase[]" value="{{ $order_detail->purchase }}" class="form-control small_dropdown">
                                </td>
                                <td>
                                    <input type="number" name="sale[]" value="{{ $order_detail->sale }}" class="form-control small_dropdown">
                                </td>
                                <td>
                                    <select name="unit[]" class="form-control small_dropdown">
                                        @foreach($units as $unit)
                                            <option {{ ($order_detail->unit == $unit->name) ? 'selected' : '' }} value="{{$unit->name}}">{{ $unit->name }}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td class="text-center">
                                    <input type="number" name="quantity[]" value="{{ $order_detail->quantity }}" class="form-control small_dropdown">
                                </td>
                                <td>
                                    {{ number_format($order_detail->total) }}
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
                    ...
                    <button type="submit" class="btn btn-success small_dropdown">Update purchase price</button>
                    </form>
                </div>
                <div class="col-md-5">
                    <!-- <div id="msg"></div> -->
